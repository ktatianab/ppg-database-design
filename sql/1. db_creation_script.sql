...
/* Created on:     22/04/2026 2:40:07ï¿½p.ï¿½m.                     */
/*==============================================================*/
...
create table AUTH_CREDENTIAL (
   ID_CREDENTIAL       INT4                 not null GENERATED BY DEFAULT AS IDENTITY,
   ID_USER             INT4                 not null,
   PASSWORD_HASH       VARCHAR(255)         not null,
   IS_ACTIVE           BOOL                 not null DEFAULT TRUE,
   CREATED_AT          TIMESTAMP            not null DEFAULT CURRENT_TIMESTAMP,
   UPDATED_AT          TIMESTAMP            not null DEFAULT CURRENT_TIMESTAMP,
   constraint PK_AUTH_CREDENTIAL primary key (ID_CREDENTIAL),
   constraint AK_AUTH_CREDENTIAL_USER unique (ID_USER)
);

comment on table AUTH_CREDENTIAL is
'Table that stores authentication credentials associated with system users';

comment on column AUTH_CREDENTIAL.PASSWORD_HASH is
'Password hash used only by the authentication microservice';

comment on column AUTH_CREDENTIAL.CREATED_AT is
'Audit Fields';

comment on column AUTH_CREDENTIAL.UPDATED_AT is
'Audit Fields';

/*==============================================================*/
/* Index: AUTH_CREDENTIAL_PK                                    */
/*==============================================================*/
create unique index AUTH_CREDENTIAL_PK on AUTH_CREDENTIAL (
ID_CREDENTIAL
);

/*==============================================================*/
/* Index: AUTH_CREDENTIAL_USER_FK                               */
/*==============================================================*/
create unique index AUTH_CREDENTIAL_USER_FK on AUTH_CREDENTIAL (
ID_USER
);
...
create table SEVERITY_LEVEL (
   ...
   NAME                 VARCHAR(255)         not null,
   constraint PK_SEVERITY_LEVEL primary key (ID_SEVERITY_LEVEL),
...
alter table ALERT
   add constraint FK_ALERT_SEVERITY__SEVERITY foreign key (ID_SEVERITY_LEVEL)
      references SEVERITY_LEVEL (ID_SEVERITY_LEVEL)
      on delete restrict on update restrict;
...
alter table AUTH_CREDENTIAL
   add constraint FK_AUTH_CREDENTIAL_APP_USER foreign key (ID_USER)
      references APP_USER (ID_USER)
      on delete restrict on update restrict;

alter table REGION
   add constraint FK_REGION_COUNTRY_REG_COUNTRY foreign key (ID_COUNTRY)
      references COUNTRY (ID_COUNTRY)
      on delete restrict on update restrict;

alter table CITY
   add constraint FK_CITY_REGION_CIT_REGION foreign key (ID_REGION)
...
alter table MONITORING_SESSION
   add constraint FK_MONITORI_COMPUTE_M_COMPUTE foreign key (ID_COMPUTE_STATUS)
